...
#colnames(rgs_cov)=rownames(rgs_cov)=colnames(phe)=rownames(phe)=paste("Trait",1:4)
...
library(corrplot)
toPlot=out_rg
#colnames(toPlot)=rownames(toPlot)=c(paste("Trait",1:4),paste0("GIP",1:4))
colnames(toPlot)=rownames(toPlot)=c(nms,paste0("GIP",1:4))
...
library(corrplot)
toPlot=out_rg
colnames(toPlot)[6:10]=rownames(toPlot)[6:10]=paste0("GIP",1:5)
...
dta=out$eigens
colnames(dta)=paste0("GPC",1:5)
rownames(dta)=colnames(out$cov_g)[1:5]

dta=cbind(dta,Trait=rownames(dta))
dta=as.data.frame(dta)
dta[,1:5]=apply(dta[,1:5],MAR=2,as.numeric)
dta=dta %>% gather(GPC, a,-Trait)

dta$GPC=as.factor(dta$GPC)
dta$GPC=factor(dta$GPC,levels(dta$GPC)[5:1])
