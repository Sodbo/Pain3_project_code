...
sd.table <- read.csv(
  file = '/mnt/polyomica/projects/mv_gwas/data/chronic_replication/AA_10102018/raw/cases.controls_repl.AA.csv',
  header = TRUE,
  sep = ';',
  skip = 1,
...
for (col.number in 2:ncol(sd.table)) {
    ...
    gwas.name <- paste('MV', pain.type, 'Repl.AA_gwas.BGEN.stats.txt', sep = "_")
    raw.gwas <- data.table::fread(
      input = paste0('/mnt/polyomica/projects/mv_gwas/data/chronic_replication/AA_10102018/raw/',gwas.name),
        data.table=F,
          header=T,
          stringsAsFactors=F)
    ...
    data.table::fwrite(gwas.standart, 
        file = paste0('/mnt/polyomica/projects/mv_gwas/data/chronic_replication/AA_10102018/scaled_filtered/', gwas.name))
